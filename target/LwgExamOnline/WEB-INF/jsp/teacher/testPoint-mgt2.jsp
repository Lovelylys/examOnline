...
<div class="table-box" id="myDiv">
    ...
    <thead>
    <th class="num"></th>
    <th class='name'>编号</th><th class="process">所属科目</th><th class="process">所属年级</th>
...
<%--<script type="text/javascript" src="${ctx}/js/jquery.pagination.js"></script> 第一套分页插件--%>
...
function queryByGradeAndCourse()
{
    ...
    $.ajax({
        url:"${ctx}/testPoint/toTestPointPage3.action",
        type:"post",
        ...
        success:function(data){
            // InitTable2(0);
            $("#specDiv").html("<nav aria-label=\"Page navigation\">" +
...
function InitPage2(total)
{
    $('.pagination').twbsPagination({
        totalPages: total,
        visiblePages: 7,
        onPageClick: InitTable2,
    });
}
function InitTable2(event,page)
{
    var gradeId = $("select[name=gradeId]").val();
    var courseId = $("select[name=courseId]").val();
    var data0={
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            for(dataList in data){
                ...
                html += "<td><input type='checkbox' name='typeId' value='"+data[dataList].id+"'/></td>";
                ...
                html += "<td class='operate'><a href='${ctx}/testPoint/delTestPoint.action?id="+data[dataList].id+"' class='del'>删除</a>&nbsp;";
                html += "<a href='${ctx}/testPoint/toUpdTestPoint.action?id="+data[dataList].id+"' class='del'>编辑</a>&nbsp;";
                html += "<a href='${ctx}/testPoint/toQryTestPoint.action?id="+data[dataList].id+"' class='del'>查看</a></td>";
                html += "</tr>";
            }
            html += "</tbody>";
            $("tbody [align=center]").remove();
