...
<div class="main">
    ...
    <div class="long-input select ue-clear">
    	...
    	<c:forEach items="${grade}" var="grade">
			<input name="gradeId" type="radio" value="${grade.gradeId}" 
				<c:if test="${grade.gradeId eq user.grade.gradeId  }">checked</c:if> />${grade.gradeName}
		</c:forEach>
    </div>
    <div class="short-input select ue-clear">
    	<label><span style="color:red">*</span>账号类型：</label>
    	<input name="userType" type="radio" value="0" <c:if test="${user.userType eq 0}">checked</c:if>/>学生
    	<input name="userType" type="radio" value="1" <c:if test="${user.userType eq 1}">checked</c:if>/>老师
    	<input name="userType" type="radio" value="0" <c:if test="${user.userType eq 2}">checked</c:if>/>管理员
    </div>
    <p class="short-input ue-clear">
    ...
    <p class="long-input ue-clear">
        <label>班级：</label>
        <input type="text" id="className" name="className" value="${examClass.className }"/>
    </p>
</div>
</form>
