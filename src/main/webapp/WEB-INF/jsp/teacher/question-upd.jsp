...
<form action="${ctx}/question/updQuestion.action" method="post" name="myform" id="myform">
<div class="main">
    ...
    <div class="long-input select ue-clear">
    	...
    	<c:forEach items="${courseList}" var="course">
			<input name="course.courseId" type="radio" value="${course.courseId}" onclick="courseOnclick()"
				<c:if test="${course.courseId eq question.course.courseId }">checked</c:if>/>${course.courseName}
		</c:forEach>
    </div>
    <div class="long-input select ue-clear">
    	<label><span style="color:red">*</span>题型：</label>
    	<c:forEach items="${typeList}" var="typeInfo">
			<input id="type.typeId" name="type.typeId" type="radio" onclick="typeOnclick()" value="${typeInfo.typeId}"
				<c:if test="${typeInfo.typeId eq question.type.typeId }">checked</c:if>/>${typeInfo.typeName}
		</c:forEach>
    </div>
...
function updQuestion(){
	document.myform.attributes["action"].value = "${ctx}/question/updQuestion.action";
	$("form").submit();
}
